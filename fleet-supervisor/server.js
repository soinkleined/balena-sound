...
// If the master server is being forced via MULTI_ROOM_SERVER then hardcode it into the masterServer process
if (process.env.MULTI_ROOM_SERVER) {
  masterServer.update(process.env.MULTI_ROOM_SERVER)
} 
// If the master server is NOT being forced we rely on fleet management events to dynamically update it
else {
  ...
  if (process.env.BALENA_DEVICE_TYPE !== 'raspberry-pi' && !process.env.CLIENT_ONLY_MULTI_ROOM && !process.env.MULTI_ROOM_SERVER) {
    snapcastServer.connect()
    snapcastServer.onPlayback(() => {
